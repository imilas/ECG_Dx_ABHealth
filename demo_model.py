...
def get_model_agsx(n_classes, last_layer='sigmoid',dropout_keep=0.8):
    kernel_size = 16
    ...
    x, y = ResidualUnit(1024, 128, kernel_size=kernel_size,
                        kernel_initializer=kernel_initializer,dropout_keep_prob=dropout_keep)([x, x])
    x, y = ResidualUnit(256, 196, kernel_size=kernel_size,
                        kernel_initializer=kernel_initializer,dropout_keep_prob=dropout_keep)([x, y])
    ...
    model = Model(inputs=[signal,agsx], outputs=diagn)
    return model 
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, ecg_labels,age_sex=False, batch_size=64,
        ...
        self.agsx_df = age_sex.set_index("ecgId")
        self.on_epoch_end()
    def __len__(self):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        X_agsx = np.zeros((self.batch_size, 2))
        y = np.zeros((self.batch_size, self.n_classes))
        for i, ID in enumerate(list_IDs_temp):
            try: 
                np_path = "./demo_data/%s.npy"%ID
                # f = gzip.GzipFile(np_path, "r")
                enp = np.load(np_path)[:,0:4096]
            ...
            try:
                ...
                print("age sex not loaded",e)
                # print("%s does not have agsx features:"%ID)
                X_agsx[i] = [64,1]
                # print("please supply age and sex dataframe to the data generator")
                pass
        return (X,X_agsx), y
